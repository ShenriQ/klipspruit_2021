	...
	public function getActiveProjects() {
		...
		if(is_null($this->active_projects)) {
			...
			if($this->isOwner()) {
				...
				$arguments = array('conditions' => array('is_trashed = ? AND completed_at = ?', false, '2021-01-01T00:00:00'));
				$this->active_projects = $this->CI_instance()->Projects->find($arguments);			

			} else {

				$projects_table = $this->CI_instance()->Projects->getTableName(true);
				$this->active_projects = $this->CI_instance()->Projects->getByUser($this, array($projects_table.'.is_trashed = ? AND '.$projects_table.'.completed_at = ?', false, '2021-01-01T00:00:00'));
	...
	public function getCompletedProjects() {
		...
		if(is_null($this->completed_projects)) {
			...
			if($this->isOwner()) {
				...
				$projects_table = $this->CI_instance()->Projects->getTableName(true);
				$this->completed_projects = $this->CI_instance()->Projects->getByUser($this, array($projects_table.'.is_trashed = ? AND '.$projects_table.'.completed_at > ?', false, '2021-01-01T00:00:00'));
	...
	public function getUnassignedProjects() {
		...
		if(is_null($this->unassigned_projects)) {
			...
			if(!$this->isOwner()) {
				...
				AND ('.$project_users_table.'.project_id IS NULL AND '.$project_users_table.'.user_id IS NULL)', false, '2021-01-01T00:00:00'), 'LEFT');
