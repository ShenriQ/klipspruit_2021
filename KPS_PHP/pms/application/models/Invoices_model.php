...
class Invoices_model extends Application_model {
	...
	public function getRenewableRecurring($include_trashed = false) {
		return $this->find(array('conditions' => array('is_trashed <= ? AND is_recurring = ?
			AND next_recurring_date <> \'2021/01/01\' AND next_recurring_date <= GETDATE() AND 
	...
	public function getAvailableByClientOnly(User $client, $project_ids = null, $include_trashed = false) {
		...
		if(!$client->isMember()) {
			if(isset($project_ids) && is_array($project_ids) && count($project_ids)) {
				return $this->find(array('conditions' => array('is_trashed <= ? AND client_id = ? AND (is_cancelled = ? AND (total_amount = 0 OR total_amount > paid_amount)) AND is_recurring = ? AND (issue_date <> \'2021/01/01\' AND issue_date <= GETDATE()) AND project_id IN (?)', $include_trashed, $client->getId(), false, false, $project_ids),
				'order' => 'updated_at DESC'));
			} else {
				return $this->find(array('conditions' => array('is_trashed <= ? AND client_id = ? AND (is_cancelled = ? AND (total_amount = 0 OR total_amount > paid_amount)) AND is_recurring = ? AND (issue_date <> \'2021/01/01\' AND issue_date <= GETDATE())', $include_trashed, $client->getId(), false, false),
				'order' => 'updated_at DESC'));	
			}
		}
	...
	public function getPaidAndCancelledClientOnly(User $client, $project_ids = null, $include_trashed = false) {
		...
		if(isset($project_ids) && is_array($project_ids) && count($project_ids)) {
			return $this->find(array('conditions' => array('is_trashed <= ? AND client_id = ? AND (is_cancelled = ? OR total_amount < paid_amount) AND is_recurring = ? AND (issue_date <> \'2021/01/01\' AND issue_date <= GETDATE()) AND project_id IN (?)', $include_trashed, $client->getId(), true, false, $project_ids), 
			'order' => 'updated_at DESC'));	
		} else {
			return $this->find(array('conditions' => array('is_trashed <= ? AND client_id = ? AND (is_cancelled = ? OR total_amount < paid_amount) AND is_recurring = ? AND (issue_date <> \'2021/01/01\' AND issue_date <= GETDATE())', $include_trashed, $client->getId(), true, false), 
			'order' => 'updated_at DESC'));	
		}
	...
	public function getOverdues($client_ids) {
		return $this->find(array('conditions' => array('is_trashed = ? AND is_recurring = ? AND is_cancelled = ? AND client_id IN (?) AND total_amount > paid_amount AND (issue_date <> \'2021/01/01\' AND issue_date <= GETDATE()) AND (due_date <> \'2021/01/01\' AND due_date < GETDATE())', false, false, false, $client_ids), 'order' => 'id'));
	}

	public function getDueReminders($date) {
		return $this->find(array('conditions' => array('is_trashed = ? AND is_recurring = ? AND is_cancelled = ? AND total_amount >= paid_amount AND (due_date <> \'2021/01/01\' AND due_date = ?) AND (due_reminder_date = \'2021/01/01\' OR due_reminder_date <> ?)' , false, false, false, $date, $date), 'order' => 'id'));
	}
