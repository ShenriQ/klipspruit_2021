	...
	public function ipn(){
		...
		if (stripos ($ipn_response, "VERIFIED") === false){
			...
			if ($ipn_data['payment_status'] == "Completed") {
				...
				$user_query = $this->db->query("SELECT * FROM `users` WHERE `id` = '".$item_number."' LIMIT 1");
				...
				&& $receiver_email == $paypal_email && $mc_currency == $paypal_currency_code) {
					...
					try{
						...
						VALUES (NULL, 'paypal', '0', ".$this->db->escape($payer_name).", '".$payer_email."', 'completed', '".$receiver_email."', '".$amount."', '".$mc_fee."', '".$tax."', '".$txn_id."', '".$mc_currency."', ".$this->db->escape($raw).", ".$this->db->escape($item_name).", NOW(), '".$user_record->id."', '".$user_record->target_source_id."');");
