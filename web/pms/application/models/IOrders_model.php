...
class IOrders_model extends Application_model {
	...
	public function getPaymentsStatsFor($start, $end) {
		...
		$query = $this->db->query("SELECT SUM(`send_amount`) AS `summary`, SUM(`payment_fee`) AS `fee_amount`, SUBSTR(`created_at`, 1, 7) AS `stats_date` FROM ".$transaction_logs_table."
		WHERE DATE(`created_at`) BETWEEN '$start' AND '$end' GROUP BY `stats_date`");
