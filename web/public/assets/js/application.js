(function ($) {
  ...
  function serialize_ajax_call(a, b, d) {
    $("#" + d).button("loading");
    $.ajax({
      url: a, type: "POST", data: b, dataType: "json", success: function () {
        $("#" + d).button("reset");
      }
    }).done(function (a) {
      if (a.success) {
  ...
  function formdata_ajax_call(a, b, d, e) {
    $("#" + d).button("loading");
    $.ajax({
      url: a, type: "POST", data: b, processData: !1, contentType: !1, dataType: "json", success: function () {
        $("#" + d).button("reset");
      }
    }).done(function (a) {
      if (a.success) {
  ...
  $(document).ready(function () {
    $(document).on("change", ".item_amount", function () {
      var a = $(this).attr("id"), a = a.replace("amount", ""), a = a.replace("_", ""), a = parseInt(a), b = parseFloat($(this).val()), b = convert_number(b);
      $("#amount_" + a).val(b);
    ...
    $(document).on("change", ".item_quantity", function () {
      var a = $(this).attr("id"), a = a.replace("quantity", ""), a = a.replace("_", ""), a = parseInt(a), b = parseFloat($("#amount_" + a).val()), b = convert_number(b);
      $("#amount_" + a).val(b);
...
function update_tax(taxNo = '') {
  var a = get_sub_total(), b = parseFloat($("#tax_rate" + taxNo).val());
...
function get_sub_total() {
  ...
  $(".item_quantity").each(function() {
    var b = $(this).attr("id"), b = b.replace("quantity", ""), b = b.replace("_", ""), b = parseInt(b), b = parseFloat($("#amount_" + b).val()), c = parseFloat($(this).val()), b = parseFloat(b * c);
    a += b;
  });
  return a;
}
...
function ganttChart (a) {
  $(function () {
    $(".gantt").gantt({
      source: a, minScale: "years", maxScale: "years", navigate: "scroll", itemsPerPage: 30, onItemClick: function (b) {
