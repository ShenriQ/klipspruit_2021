...
class Estimates_model extends Application_model {
	...
	public function getOverdues($client_ids) {
		return $this->find(array('conditions' => array('`is_trashed` = ? AND `status` = ? AND `client_id` IN (?) AND (`due_date` <> \'0000-00-00\' AND `due_date` < NOW())', false, true, $client_ids), 'order' => '`id`'));
	}
