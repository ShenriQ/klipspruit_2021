...
class TransactionLogs_model extends Application_model {
	...
	public function getTransactionStatsFor($transaction_type, $start, $end) {
		...
		$query = $this->db->query("SELECT SUM(`amount`) AS `summary`, SUBSTR(`created_at`, 1, 7) AS `stats_date` FROM ".$transaction_logs_table."
		WHERE `transaction_type` = '".$transaction_type."' AND DATE(`created_at`) BETWEEN '$start' AND '$end' AND `target_source_id` = ".get_target_source_id()." GROUP BY `stats_date`");
